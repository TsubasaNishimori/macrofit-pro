...
| キャッシュクリア | UIからセッション/ローカルストレージをワンクリック初期化 |
...
| 体重週数が日付とズレる | 再現時: キャッシュクリア → 入力再送信（ロジックは修正済） |
| 生成失敗（APIエラー） | `.env.local` のキー確認 / 利用制限ログをAzureポータルで確認 |
| 画面が古いまま | ブラウザキャッシュ削除 or `🧹 キャッシュクリア` |
